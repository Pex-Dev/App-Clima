...
function mostrarIconosCarga(){
    ...
    for (let index = 0; index < 14 ; index++) {
        ...
        dia.innerHTML = '<img class="icono-carga" src="src/img/loading-icon.gif" alt="Icono resumen clima">';
        contenedorDias.appendChild(dia); 
    }
    eliminarHorasCargadas();
    for (let index = 0; index < 24 ; index++) {
        ...
        hora.innerHTML = '<img class="icono-carga" src="src/img/loading-icon.gif" alt="Icono resumen clima">';
    ...
    imagenClima.setAttribute('src','src/img/loading-icon.gif');
}
...
async function obtenerDatosDesdeAPI(lat,lon){
    const cordenadas = new FormData();
    cordenadas.append('latitud',lat);
    cordenadas.append('longitud',lon);

    try {
        const url = `${location.origin}/php/obtenerPronostico.php`;
